...
label say_line(role=None, text=""):
    if role:
        ...
        $ renpy.say(None, text)
    return
label play_scene(dialogue,menu_li = []):
    python:
        menu_idx = 0
    ...
    while idx < (len(dialogue)):
        $ role, text, bg_idx_li, char_info, is_menu, is_hide_text = dialogue[idx]
        ...
        if bgm_idx == 0 and current_bgm_idx != 0:
            ...
            $ current_bgm_idx = bgm_idx
        # BGS变化
        if bgs_idx != current_bgs_idx:
            $ bgs = bgs_li[bgs_idx]
            play sound bgs fadein 1.5 loop
        ...
        if last_state != char_info:
            ...
            while(i < 6):
                ...
                # 状态没变，跳过
                if last_state[i] != char_info[i]:
                    

                    # ch_pos == 0 且 ch_show != 0，强制ch_show=0
                    ...
                    if ch_show != 0:
                        ...
                        if ch_move != ch_pos and 1 <= ch_move <= 5:
                            ...
                            $ show_name = f"{role_name} {e_motion}"
                            if i == 0:
...
label start:
    ...
    call play_scene(charpter3_text_li,menu_dict["charpter3"])
    call play_scene(charpter4_text_li,menu_dict["charpter4"])
    return
